export interface PicturesData {
    text: string;
    className: string;
  }
  
  export const PicturesData: PicturesData[] = [
    { 
        text: "1", 
        className: "flex items-center justify-center bg-red-500"
    },
    {
      text: "2",
      className: "flex items-center justify-center bg-blue-500"
    },
    { 
        text: "3",
        className: "flex items-center justify-center bg-green-500"
    },
    {
      text: "4",
      className: "flex items-center justify-center bg-yellow-500"
    },
    {
      text: "5",
      className: "flex items-center justify-center bg-purple-500"
    },
    {
      text: "6",
      className: "flex items-center justify-center bg-pink-500"
    },
    {
      text: "7",
      className: "flex items-center justify-center bg-indigo-500"
    },
    {
      text: "8",
      className: "flex items-center justify-center bg-leap-dark-green"
    },
    {
      text: "9",
      className: "flex items-center justify-center bg-leap-grey lg:col-span-2"
    },
    {
      text: "10",
      className: "flex items-center justify-center bg-leap-light-green lg:col-span-2"
    },
    {
      text: "11",
      className: "flex items-center justify-center bg-red-700 lg:col-span-2"
    },
    {
      text: "12",
      className: "flex items-center justify-center bg-leap-yellow lg:col-span-2"
    },
    {
      text: "13",
      className: "flex items-center justify-center bg-blue-800 md:col-span-2 lg:col-span-3 lg:row-span-2"
    },
    {
      text: "14",
      className: "col-span-2 flex items-center justify-center bg-teal-400 sm:col-span-2 md:col-span-2 lg:col-span-2 lg:row-span-2"
    },
    {
      text: "15",
      className: "flex items-center justify-center bg-purple-700 md:col-span-2 lg:col-span-3 lg:row-span-2"
    },
    {
      text: "16",
      className: "col-span-4 row-span-2 flex items-center justify-center bg-leap-mid-green md:col-span-3 lg:col-span-4 lg:row-span-3"
    },
    {
      text: "17",
      className: "col-span-4 row-span-2 flex items-center justify-center bg-leap-dark-green md:col-span-3 lg:col-span-4 lg:row-span-3"
    }
  ];
  